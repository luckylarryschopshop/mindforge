...
@app.get("/api/mindforge")
async def list_items():
    return _load()

@app.post("/api/mindforge")
...
@app.delete("/api/mindforge/{item_id}")
async def delete_item(item_id: int):
    items = [i for i in _load() if i.get("id") != item_id]
    _save(items)
    return {"status": "deleted"}

@app.get("/api/mindforge/search")
